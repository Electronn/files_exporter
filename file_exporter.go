...
//  if params.Get("regexp") == "" { http.Error(w, "regexp param is missing", http.StatusBadRequest) }
  ...
  fmt.Fprintf(w, "# file_match_regex statuses : \n# 1 - regexp set and found in file \n# 0 - regexp set and not found \n# -1 - regexp not set\n")
  fmt.Fprintf(w, "file_match_regex: " + MatchStringFile(params.Get("target"), regexp) + "\n\n")
  fmt.Fprintf(w, "# file_md5_dec is decimal equivalent of md5 file. Calculated by the direct number convertion from hex to decimal, because Prometheus can store only numbers \n# Real md5 checksum is: " + MD5SumFile(params.Get("target")) + "\n")
  fmt.Fprintf(w, "file_md5_dec: " + fmt.Sprintf("%f", md5tofloat64(MD5SumFile(params.Get("target")))))
